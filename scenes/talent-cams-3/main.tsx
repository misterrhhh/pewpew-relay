...
function TalentCams3Scene() {
	const [talent, setTalent] = useState<Talent[]>([]);
	const [scene, setScene] = useState<TalentCamsSceneState>(defaultSceneState);

	useEffect(() => {
		void Promise.all([api.listTalent(), api.getTalentCams3Scene()]).then(([nextTalent, nextScene]) => {
			setTalent(nextTalent);
			setScene(nextScene);
		});

		const socket = connectSceneSocket((message) => {
			if (message.scene === "talentCams3") {
	...
	const selectedTalent = useMemo(
		() => scene.talentIds.slice(0, 3).map((talentId) => talent.find((entry) => entry.id === talentId) ?? null),
...
ReactDOM.createRoot(document.getElementById("root")!).render(<TalentCams3Scene />);
