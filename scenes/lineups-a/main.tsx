...
function LineupsAScene() {
	const [teams, setTeams] = useState<TeamResponse[]>([]);
	const [scene, setScene] = useState<LineupsSceneState>(defaultSceneState);

	useEffect(() => {
		function loadData() {
			return Promise.all([api.listTeams(), api.getLineupsAScene()]).then(([nextTeams, nextScene]) => {
				setTeams(nextTeams);
				setScene(nextScene);
			});
		}
...
ReactDOM.createRoot(document.getElementById("root")!).render(<LineupsAScene />);
