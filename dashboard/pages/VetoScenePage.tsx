...
export function VetoScenePage({
  ...
  async function pushUpdate(next: Partial<VetoSceneState>) {
    try {
      const response = await api.updateVetoScene(next);
      setScene(response);
  ...
  async function handleSaveVetos() {
    ...
    try {
      ...
      const nextScene = await api.updateVetoScene({ ...scene });
      setScene(nextScene);
      await refresh();
    ...
    <section className="page">
      ...
      <div className="card-grid scene-dashboard-grid">
          ...
          <div className="form-grid">
            ...
            <div className="field">
              ...
              <input
                ...
                min="0"
                max="7"
                value={scene.currentIndex}
                onChange={(event) => setScene({ ...scene, currentIndex: Math.max(0, Math.min(7, Number(event.target.value) || 0)) })}
              />
            </div>
        ...
        <div className="panel scene-preview-panel">
          <h3>Preview</h3>
          <p>{previewUrl}</p>
          <p>{selectedMatch ? matchLabel(selectedMatch) : "No match selected."}</p>
          <p>Current step: {scene.currentIndex}</p>
          <div className="actions">
